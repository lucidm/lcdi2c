    ...
    if (argc == 1)
    {
        fprintf(stderr, "Wrong number of arguments.\n\tProper command call: "
	       "%s <bus number> <hex address> <topology>\n"
               "\tWhere bus number is number of bus LCD is connected to, default set to: %d\n"
               "\taddress is address of the I2C expander in hexadecimal, default set to: 0x%02X\n"
               "\ttopology depends on your LCD configuration and is one of below:\n"
               "\t\t0 - 40x2\n"
               "\t\t1 - 20x4\n" 
               "\t\t2 - 20x2\n" 
               "\t\t3 - 16x4\n" 
