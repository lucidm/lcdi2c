...
static void set_welcome_message(LcdHandler_t *lcdData, char *wscreen) {
    if (strlen(wscreen)) {
        strncpy(lcdData->welcome, wscreen, WS_MAX_LEN);
    } else {
        strncpy(lcdData->welcome, DEFAULT_WS, WS_MAX_LEN);
    }
}

static int lcdi2c_probe(struct i2c_client *client, const struct i2c_device_id *id) {
    LcdHandler_t *lcdData = (LcdHandler_t *) devm_kzalloc(&client->dev, sizeof(LcdHandler_t), GFP_KERNEL);

    if (!lcdData)
        return -ENOMEM;

    i2c_set_clientdata(client, lcdData);
    sema_init(&lcdData->sem, 1);
    lcdData->handle = client;
    lcdData->backlight = 1;
    lcdData->cursor = cursor;
    lcdData->blink = blink;
    lcdData->deviceopencnt = 0;
    lcdData->major = major;
    lcdData->swscreen = swscreen;

    set_welcome_message(lcdData, wscreen);

    lcdinit(lcdData, topo);
    if (lcdData->swscreen) {
        lcdprint(lcdData, lcdData->welcome);
    }

    dev_info(&client->dev, "%ux%u LCD using bus 0x%X, at address 0x%X",
             lcdData->organization.columns,
             lcdData->organization.rows, busno, address);
    return 0;
}
...
static ssize_t lcdi2c_customchar(struct device *dev,
    ...
    for (int i = 0; i < count; i += 9) {
        if (buf[i] > 7) {
            dev_err(&gClient->dev, "%d is out of range, valid range is 0-7\n", buf[i]);
            up(&gData->sem);
            return -ETOOSMALL;
        }
